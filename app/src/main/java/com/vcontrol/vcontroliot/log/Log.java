...
package com.vcontrol.vcontroliot.log;
...
public class Log
{
    ...
    public static int startLogService(String szLogPath, String szLogName, int nMaxSize)
    {
        ...
        CommConfig.LOG_FILEPATH = szLogPath;
        CommConfig.LOG_NAME = szLogName;
        CommConfig.LOG_MAXSIZE = nMaxSize;
        Log.startLogFile();
        return CommConfig.METHOD_SUCCESS;
    }
    ...
    private static void initLog()
    {
        try
        {
            MAX_SIZE = CommConfig.LOG_MAXSIZE;//设置日志大小
            new File(CommConfig.LOG_FILEPATH).mkdirs();//创建目录
            logFile = new File(CommConfig.LOG_FILEPATH + CommConfig.LOG_NAME);
            if (!logFile.exists())
            {//日志文件不存在，则创建
    ...
    private static void checkLogSize()
    {
        try
        {
            ...
            if (size >= MAX_SIZE)
            {//超出大小
                logFile.renameTo(new File(CommConfig.LOG_FILEPATH + CommConfig.LOG_NAME + bakLogName));//改名
                logFile.delete();
                initLog();//重新初始化
            }
        }
    ...
    public static void d(String msg)
    {
        ...
        android.util.Log.d(TAG, "(" + targetStackTraceElement.getFileName() + ":"
