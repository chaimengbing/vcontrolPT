...
public class RetrofitServiceInstance
{
    ...
    public static RetrofitService getInstance()
    {
        ...
        {//全局拦截器，往请求头部添加token字段，就实现了全局token
            @Override
            public Response intercept(Chain chain) throws IOException
            {
                Request originalRequest = chain.request();
                Request tokenRequest = null;
//                VcontrolApplication application = new VcontrolApplication();
//                if (TextUtils.isEmpty(VcontrolApplication.getToken()))
//                {
//                    Log.d("MyApplication 的token值为空！！！！！！！！！！！！！");
//                    return chain.proceed(originalRequest);
//                }
                tokenRequest = originalRequest.newBuilder()
                        .header("token", VcontrolApplication.getToken())
                        .build();
            ...
            synchronized (RetrofitService.class)
            {
                if (instance == null)
                {
                    Retrofit retrofit = new Retrofit.Builder()                              //生成实例
                            ...
                            .client(client)
                            .addConverterFactory(GsonConverterFactory.create(new Gson()))            //使用Gson解析
